    ...
    public class DbServerFixture : IDisposable
    {
        ...
        public DbServerFixture()
        {
            ...
            TestSettings = new TestSettings()
            {
                ...
                TestDataSettingsFilePath = config.GetSection(TestSettings.Position)["TestDataSettingsFilePath"],
                TestDataSettingsDevelopmentFilePath = config.GetSection(TestSettings.Position)["TestDataSettingsDevelopmentFilePath"],
                TestDataSettingsMobileFilePath = config.GetSection(TestSettings.Position)["TestDataSettingsMobileFilePath"],
                TestDataSettingsMobileDevelopmentFilePath = config.GetSection(TestSettings.Position)["TestDataSettingsMobileDevelopmentFilePath"],
                IsGithubAction = IsGithubAction
            };

            if (TestSettings.IsDockerComposeRequired && !TestSettings.IsGithubAction)
            {
                dockerStarter = new DockerStarter(TestSettings.DockerComposeExePath, TestSettings.DockerComposeFile, TestSettings.DockerWorkingDir, 40 * 1000);
                dockerStarter.Start();
            }
        }
